...
def parse_args():
    ...
    # meta_data.json 还是照用，只是变成通过 video_path 来索引 dim_list
    parser.add_argument(
        "--meta_file",
        type=str,
        default="meta_data.json",
        help="meta_data.json 文件名（位于 root_dir 同级目录下）",
    )

    # 新增：episodic_dataset_fixed_static.npy 路径
    parser.add_argument(
        "--episode_npy",
        type=str,
        default="/home/v-zhifeng/HPE/v-zhifeng/agibot_beta_split_500/episodic_dataset_fixed_static.npy",
        help="episodic_dataset_fixed_static.npy 的完整路径",
    )

    parser.add_argument(
        "--output_json",
        type=str,
        default="dataset_stats_mp_static.json",
        help="输出的 norm_stats json 文件名（保存在 root_dir 下）",
    )
...
def process_episode(args):
    """
    Worker 函数：处理单个「片段」，返回统计量的中间结果 (Sum, SqSum, Count)
    参数 args 是一个 tuple: (root_dir, ep_key, dim_list, start_idx, end_idx)

    ep_key   : 例如 "445/808851"
    start_idx, end_idx : episodic_dataset_fixed_static.npy 里的 start_end 一行
    ...
    """
    root_dir, ep_key, dim_list, start_idx, end_idx = args
    ...
    # ===== 用 start_idx / end_idx 截取静态片段 =====
    # 这里假设 start_end 的含义是 [start, end)（end 为「最后一帧的下一个索引」），
    # 如果你那边是 [start, end]（end 包含），可以改成 end_idx = min(end_idx + 1, T_total)
    ...
    end_idx = max(start_idx + 1, min(end_idx, T_total))

    joints_seg = joints[start_idx:end_idx]
    T = joints_seg.shape[0]
    if T < 2:
        return None

    # --- 1. State Stats (Joints Abs) ---
    s_sum = np.sum(joints_seg, axis=0)
    s_sq_sum = np.sum(joints_seg ** 2, axis=0)
    s_count = T

    # --- 2. Action Stats (Joints Delta) ---
    joint_delta = joints_seg[1:] - joints_seg[:-1]
    ...
    if T > STATE_SAMPLES_PER_EP:
        ...
        sample_state = joints_seg[idx_state]
    else:
        sample_state = joints_seg.copy()
...
def main():
    ...
    # 2. 加载 episodic_dataset_fixed_static.npy
    print(f"📖 Loading episodic info from {args.episode_npy}...")
    epi = np.load(args.episode_npy, allow_pickle=True).item()
    video_paths = epi["video_path"]   # (N,)
    start_end = epi["start_end"]      # (N, 2)

    assert len(video_paths) == start_end.shape[0], "video_path 和 start_end 长度不一致"

    total_segments = len(video_paths)
    print(f"📌 Found {total_segments} segments in episodic_dataset_fixed_static.npy")

    # 3. 准备任务参数列表
    tasks = []
    missing_in_meta = 0
    for i in range(total_segments):
        ep_key = str(video_paths[i])  # 例如 "445/808851"
        s_idx, e_idx = start_end[i]

        if ep_key not in meta_data:
            missing_in_meta += 1
            continue

        dim_list = meta_data[ep_key]["dim_list"]
        tasks.append((args.root_dir, ep_key, dim_list, s_idx, e_idx))

    if missing_in_meta > 0:
        print(f"⚠️  {missing_in_meta} segments' video_path 不在 meta_data.json 中，被跳过。")
    print(f"✅ Valid segments to process: {len(tasks)}")
    ...
    print(f"✅ Processed {valid_files} valid segments.")
